...
The test checks native window resize support, new screen and viewport dimensions after wider/taller/smaller/larger resizes, the minimum size, actual rendered pixels at all four corners and quadrant centers, framebuffer copy dimensions, OpenGL errors, and a newly initialized graphics backend using its requested resolution. It also checks that unchanged dimensions do not repeatedly request resource rebuilds. The backend reinitialization case does not load or save game configuration.

Borderless coverage checks the restored automatic behavior when a windowed launch uses the selected monitor's exact desktop resolution or `0,0`. It checks explicit borderless at desktop and smaller resolutions, native and OpenGL dimensions, full monitor placement including the taskbar area, rejection of native sizing, and unchanged display resolution/refresh rate. Desktop borderless cases run on every connected monitor. An explicit bordered override at desktop size must retain working resize borders; growing an ordinary window to desktop dimensions must retain its controls. Fullscreen takes priority over a remembered borderless preference.
...
4. Quit normally and inspect the configured width and height. They should retain their launch values. Relaunch and confirm the window starts at those values. With no explicit border preference, repeat with width and height set to `0,0` or the monitor's native resolution: launch fills the monitor without borders while remaining windowed, and saving keeps the configured resolution.
5. Change resolution deliberately through the graphics options, then quit and relaunch. Confirm that explicit options changes still persist. Enable Borderless at a smaller resolution and confirm the chosen size remains unchanged without decoration. Disable Borderless at native resolution and confirm borders return after restarting. Launch fullscreen and confirm its existing behavior is unchanged, including with a remembered borderless preference.
