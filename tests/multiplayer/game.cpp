...
public:
    ...
    void Update(float dt) {
        ...
        if(role=="font") {
            ...
            tString fontError;
            if(!CheckFontEditing(menu,font,fontError)) {fail(fontError);return;}
            if(!CheckFontHintIcons(font,fontError)) {fail(fontError);return;}
            ...
            cWidgetTextBox* selected=menu->CreateWidgetTextBox(cVector3f(35,270,200),cVector2f(260,45),L"AVATAR");
            selected->SetDefaultFontType(font);
            selected->SetDefaultFontSize(cVector2f(32));
            selected->SetSelectedText(1,3);
        ...
        if(state==74) {
            ...
            if(screenshots>0) {mark(role+"-player-model-images.txt","front/side views and 96 native idle/walking transition frames with clip telemetry saved");state=75;}
            return;
        }
        if(state==75) {
