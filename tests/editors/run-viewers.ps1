...
try {
    ...
    foreach($kind in @('ModelView','MapView')) {
        $directory=Join-Path $repo ('HPL2/tools/'+$kind.ToLowerInvariant())
        $output=Join-Path $repo "bld/viewer-window-tests/$kind"
        New-Item -ItemType Directory -Path $output -Force | Out-Null
        $objects=@("$directory/x64/Debug/SimpleCamera.obj")
        & $context.Compiler /nologo /EHsc /std:c++14 /MDd /D_DEBUG /DMEMORY_MANAGER_ACTIVE /DUSE_SDL2 /DUSE_GAMEPAD /DGLEW_STATIC /D_NEWTON_USE_LIB /DIL_STATIC_LIB /DHAVE_LIBC /DAL_LIBTYPE_STATIC ('/DVIEWER_SOURCE="'+$directory+'/'+$kind+'.cpp"') "/I$directory" "/I$repo/HPL2/tools/editors/common" "/I$repo/HPL2/core/include" "/I$repo/HPL2/dependencies/include" "$PSScriptRoot/viewers.cpp" "/Fo$output/resize.obj" "/Fe$output/resize.exe" @objects @libraries /link /INCREMENTAL:NO /SUBSYSTEM:CONSOLE opengl32.lib dbghelp.lib winmm.lib setupapi.lib Imm32.lib Version.lib Avrt.lib user32.lib gdi32.lib shell32.lib ole32.lib oleaut32.lib advapi32.lib uuid.lib ws2_32.lib comctl32.lib
