...
@ifdef UseEllipsoidNormals
	uniform vec3 avEllipsoidNormalScale;
@endif
...
void main()
{
	...
	@ifdef UseNormals
		@ifdef UseEllipsoidNormals
			// The ellipsoid gradient is linear in local position. Interpolate it
			// unnormalized, then normalize per fragment for a smooth analytic normal.
			gvNormal = gl_NormalMatrix * (gl_Vertex.xyz * avEllipsoidNormalScale);
		@else
	...
	@ifdef UseNormalMapping
		...
		@ifdef UseEllipsoidNormals
			vec3 vEllipsoidNormal = normalize(gvNormal);
			gvTangent = normalize(gvTangent - vEllipsoidNormal * dot(gvTangent, vEllipsoidNormal));
			gvBinormal = normalize(cross(vEllipsoidNormal, gvTangent) * gl_MultiTexCoord1.w);
		@else
	...
	@ifdef UseParallax
		...
		@ifdef UseEllipsoidNormals
			gvTangentEyePos.z = dot(-vViewEyeVec, normalize(gvNormal));
		@else
