...
static void CheckPlayerIdentities() {
    ...
    // Both features must remain independently routable after merging branches
    // that originally assigned the same packet ID to each message.
    const auto removal=WritePropRemoval({7,42,1,"removed_prop"});
    assert(packet.front()==PlayerIdentities && removal.front()==EntityRemoved);
    assert(packet.front()!=removal.front());
