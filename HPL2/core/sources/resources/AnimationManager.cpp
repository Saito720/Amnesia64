...
namespace hpl {
	...
	cAnimation* cAnimationManager::CreateAnimation(const tString& asName)
	{
		...
		// Some distributions retain the cache but omit the original FBX source.
		if(pAnimation == NULL && sPath.empty() && cString::ToLowerCase(cString::GetFileExt(asNewName)) == "fbx")
		{
			asNewName = cString::SetFileExt(asNewName, "anm");
			pAnimation = static_cast<cAnimation*>(FindLoadedResource(asNewName, sPath));
		}
