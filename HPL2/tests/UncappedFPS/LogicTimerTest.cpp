...
int main()
{
    ...
    gTime = 0;
    hpl::cLogicTimer resetInUpdate(60, 0, Clock);
    gTime = 17;
    int resetSteps = 0;
    while(resetInUpdate.WantUpdate()) {
        Check(++resetSteps <= 6, "reset inside a slow update must not defeat the catch-up budget");
        resetInUpdate.Reset();
        gTime += 20; // Work performed by a loading/script callback after reset.
    }
    resetInUpdate.EndUpdateLoop();
    Check(resetSteps == 1, "an in-update reset discards old debt without sampling the same frame again");
    Check(Frame(resetInUpdate) == 1, "the next frame accounts for time after an in-update reset");
