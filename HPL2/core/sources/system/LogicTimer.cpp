...
namespace hpl {
	...
	void cLogicTimer::Reset()
	{
		...
		mfAccumulator = 0.0;
		// Loading/scripts may reset the clock inside an update. Keep that frame's
		// clock sample and catch-up budget until EndUpdateLoop, or a slow callback
		// which resets every tick could prevent the engine from ever rendering.
		if(!mbInUpdateLoop) mlUpdateCount = 0;
	}
