...
static void TestWorldSpaceCullingSplit()
{
	std::vector<std::unique_ptr<cDummyRenderable> > objects;
	cRenderableContainer_DynBoxTree container;
	for(int i=0; i<60; ++i)
	{
		objects.emplace_back(new cDummyRenderable("interpolated tree fixture"));
		objects.back()->GetBoundingVolume()->SetSize(cVector3f(0.5f));
		objects.back()->SetPosition(cVector3f(100.0f + i*2.0f,0,0));
		container.Add(objects.back().get());
	}
	iEntity3D::BeginRenderInterpolation(0.5f);
	container.RebuildNodes();
	container.UpdateBeforeRendering();
	container.GetRoot()->UpdateBeforeUse();
	// Interpolation bounds store world-space min/max with an identity matrix.
	// Their transform translation is zero, but their spatial split must remain
	// near this distant cluster rather than degenerating into an unsplit list.
	Require(container.GetRoot()->HasChildNodes(),"interpolated culling tree splits a distant cluster around its world-space center");
	Require(container.GetRoot()->GetMin().x > 99,"interpolated culling bounds remain in world space");
	iEntity3D::EndRenderInterpolation();
	for(auto& object : objects) container.Remove(object.get());
}
