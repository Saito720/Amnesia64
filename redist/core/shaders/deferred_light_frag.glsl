...
//Uniform varaibles
@ifdef LightType_Sun
	uniform vec3 avLightDirection;
	uniform vec4 avLightColor;
	uniform float afSunHighlightKnee;
@else
	@ifdef UseBatching
		//Do nothing
	@else
...
//Spotlight specfics
@ifdef LightType_Sun
	//Sunlight has no projection, gobo or shadow map in the first milestone.
...
@ifdef LightType_Sun
	//No additional texture is required for a Sun.
//Spot light
@elseif LightType_Spot
	@ifdef UseGobo
...
void main()
{
	...
	//The Sun pass covers the screen instead of a finite light volume. Black
	//depth is the explicit marker for pixels where the G-buffer has no object.
	@ifdef LightType_Sun
		if(dot(vDepthVal.xyz, vDepthVal.xyz) == 0.0) discard;
	@endif
	...
	@ifdef LightType_Sun
		vec3 vLightDir = normalize(avLightDirection);
	...
	@ifdef LightType_Sun
		//Directional sunlight is spatially constant.
	@elseif LightType_Spot
		@ifdef UseGobo
	...
	// Encoded normals are not guaranteed to remain exactly unit length after
	// storage and decoding. Sunlight exposes that error as camera-relative bands
	// over broad smooth surfaces, so always renormalize the directional pass.
	@ifdef UseSpecular || LightType_Sun
	...
	// HPL2 accumulates lighting into an LDR target. Preserve hue and values below
	// the knee, then smoothly compress brighter Sun contributions toward white
	// instead of allowing individual RGB channels to hard-clip.
	@ifdef LightType_Sun
		float fPeakColor = max(max(gl_FragColor.r, gl_FragColor.g), gl_FragColor.b);
		if(afSunHighlightKnee < 1.0 && fPeakColor > afSunHighlightKnee)
		{
			float fShoulderRange = 1.0 - afSunHighlightKnee;
			float fAboveKnee = fPeakColor - afSunHighlightKnee;
			float fMappedPeak = afSunHighlightKnee +
				(fAboveKnee * fShoulderRange) / (fAboveKnee + fShoulderRange);
			gl_FragColor.xyz *= fMappedPeak / fPeakColor;
		}
	@endif
